...
from paperpilot_common.core.management.utils import find_formatters, handle_extensions, run_formatters
from paperpilot_common.template import Context, Engine
from paperpilot_common.utils import archive
from paperpilot_common.utils.http import parse_header_parameters
from paperpilot_common.utils.version import get_docs_version


class TemplateCommand(BaseCommand):
    """
    Copy either a Paperpilot application layout template or a Paperpilot project
    layout template into the specified directory.

    :param style: A color style object (see paperpilot_common.core.management.color).
    ...
    def handle(self, app_or_project, name, target=None, **options):
        ...
        context = Context(
            {
                **options,
                ...
                "paperpilot_common_version": paperpilot_common.__version__,
            },
            autoescape=False,
        )
        ...
        for root, dirs, files in os.walk(template_dir):
            ...
            for filename in files:
                ...
                # accidentally render Paperpilot templates files
    ...
    def handle_template(self, template, subdir):
        ...
        Use paperpilot_common.__path__[0] as the default because the Paperpilot install
        directory isn't known.
        """
        if template is None:
            return os.path.join(paperpilot_common.__path__[0], "conf", subdir)
        else:
    ...
    def download(self, url):
        ...
        prefix = "paperpilot_common_%s_template_" % self.app_or_project
        ...
        opener = build_opener()
        opener.addheaders = [("User-Agent", f"Paperpilot/{paperpilot_common.__version__}")]
        try:
