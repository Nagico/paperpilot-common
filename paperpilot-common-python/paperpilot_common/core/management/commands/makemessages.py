...
from paperpilot_common.core.files.temp import NamedTemporaryFile
from paperpilot_common.core.management.base import BaseCommand, CommandError
from paperpilot_common.core.management.utils import find_command, handle_extensions, is_ignored_path, popen_wrapper
...
from paperpilot_common.utils.jslex import prepare_js_for_gettext
from paperpilot_common.utils.regex_helper import _lazy_re_compile
from paperpilot_common.utils.text import get_text_list
from paperpilot_common.utils.translation import templatize
...
class BuildFile:
    ...
    def is_templatized(self):
        if self.domain == "paperpilot_commonjs":
            return self.command.gettext_version < (0, 18, 3)
        elif self.domain == "paperpilot_common":
    ...
    def work_path(self):
        ...
        if not self.is_templatized:
            return self.path
        extension = {
            "paperpilot_commonjs": "c",
            "paperpilot_common": "py",
        }.get(self.domain)
...
class Command(BaseCommand):
    help = (
        ...
        "conf/locale (in the paperpilot_common tree) or locale (for projects and applications) "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--domain",
            "-d",
            default="paperpilot_common",
            help='The domain of the message files (default: "paperpilot_common").',
        )
        parser.add_argument(
            "--all",
            "-a",
        ...
        parser.add_argument(
            ...
            'if the domain is "paperpilot_commonjs"). Separate multiple extensions with '
    ...
    def handle(self, *args, **options):
        ...
        if self.domain not in ("paperpilot_common", "paperpilot_commonjs"):
            raise CommandError(
                "currently makemessages only supports domains " "'paperpilot_common' and 'paperpilot_commonjs'"
            )
        if self.domain == "paperpilot_commonjs":
        ...
        self.invoked_for_paperpilot_common = False
        self.locale_paths = []
        self.default_locale_path = None
        if os.path.isdir(os.path.join("conf", "locale")):
            self.locale_paths = [os.path.abspath(os.path.join("conf", "locale"))]
            self.default_locale_path = self.locale_paths[0]
            self.invoked_for_paperpilot_common = True
        else:
    ...
    def process_locale_dir(self, locale_dir, files):
        ...
        for translatable in files:
            ...
            if self.domain not in ("paperpilot_commonjs", "paperpilot_common"):
                continue
    ...
    def write_po_file(self, potfile, locale):
        ...
        if os.path.exists(pofile):
            ...
            with open(potfile, encoding="utf-8") as fp:
                msgs = fp.read()
            if not self.invoked_for_paperpilot_common:
    ...
    def copy_plural_forms(self, msgs, locale):
        """
        Copy plural forms header contents from a Paperpilot catalog of locale to
        the msgs string, inserting it at the right place. msgs should be the
        contents of a newly created .po file.
        """
        paperpilot_common_dir = os.path.normpath(os.path.join(os.path.dirname(paperpilot_common.__file__)))
        if self.domain == "paperpilot_commonjs":
            domains = ("paperpilot_commonjs", "paperpilot_common")
        else:
            domains = ("paperpilot_common",)
        for domain in domains:
            paperpilot_common_po = os.path.join(
                paperpilot_common_dir, "conf", "locale", locale, "LC_MESSAGES", "%s.po" % domain
            )
            if os.path.exists(paperpilot_common_po):
                with open(paperpilot_common_po, encoding="utf-8") as fp:
