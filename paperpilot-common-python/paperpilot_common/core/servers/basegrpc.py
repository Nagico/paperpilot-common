...
from paperpilot_common.core.grpc import get_grpc_application
...
def get_internal_grpc_application():
    """
    ...
    app_path = getattr(settings, "GRPC_APPLICATION", None)
    if app_path is None:
        return get_grpc_application()

    try:
        return import_string(app_path)
    except ImportError as err:
        raise ImproperlyConfigured(
            "GRPC application '%s' could not be loaded; " "Error importing module." % app_path
        ) from err
...
async def run_async(
    ...
    server = server_cls(
        ThreadPoolExecutor(max_workers=10),  # migration_thread_pool
        (),  # handlers
        grpc_handler.middleware_list,  # interceptors
        (),  # options
        100,  # maximum_concurrent_rpcs
        None,  # compression
    )
